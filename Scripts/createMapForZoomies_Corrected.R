...
for (myAng in c(90, 110, 70, 50, 30)) {
  for (mySD in c(5, 10, 20)) {  
#    for(townPos in seq(-5,5)) {
      ...
      x1 <- stX
      y1 <- stY
      x2 <- stX + numMovesCone * deltaX[myAng]
      y2 <- stY + numMovesCone * deltaY[myAng]
      coneLength <- sqrt((numMovesCone * deltaX[myAng])^2 + (numMovesCone * deltaY[myAng])^2)
      x3 <- stX + numMovesCone * (deltaX[myAng] + (mySD * .97))
      x4 <- stX + numMovesCone * (deltaX[myAng] - (mySD * .97))
      y3 <- stY + sqrt(coneLength^2 - (x3-x1)^2)
      y4 <- stY + sqrt(coneLength^2 - (x4-x1)^2)

      segments(x1,y1,x3,y3, lwd = coneLWD)
      segments(x1,y1,x4,y4, lwd = coneLWD)
      
      ang1 <- atan((y3-y2)/(x3-x2)) * 180 / pi
      ang2 <- atan((y4-y2)/(x4-x2)) * 180 / pi + 180
      myRadius <- sqrt((x4-x2)^2 + (y4-y2)^2)
      
      plotrix::draw.arc(x = x2, 
                        y = y2,
                        radius = myRadius*1.5,
                        deg1 = ang1, deg2 = ang2,
                        col="red",
